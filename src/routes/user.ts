  ...
  if (redirectUri && clientId) {
    ...
    if (sessionCookie) {
      ...
      if (session && session.expiresAt > new Date()) {
        ...
        return showConsentScreen(c, session.user, clientId, redirectUri, scope, state);
      }
    }
  }
...
userRoutes.post(
  ...
  async (c) => {
    const { email, password, redirect_uri, client_id, state, scope } = c.req.valid("form");
...
userRoutes.post(
  ...
  async (c) => {
    const { email, password, name, redirect_uri, client_id, state, scope } = c.req.valid("form");
